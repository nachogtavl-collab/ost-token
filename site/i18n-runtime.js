...
(function () {
  ...
  var DICT = {
    ...
    // Unified offline / survival vault
    'Unified vault': 'Bóveda unificada',
    'Offline OST now lives in one Survival Vault': 'OST sin internet ahora vive en una sola Bóveda de Supervivencia',
    'Mint bearer notes, import paper QR / NFC / digital files, and use that same balance in offline fair games from the Survival OST section.':
      'Minta notas al portador, importa QR en papel / NFC / archivos digitales y usa ese mismo saldo en juegos justos sin internet desde la sección Survival OST.',
    'Open Survival OST Vault': 'Abrir Bóveda Survival OST',
    'One offline coin, one local balance': 'Una moneda offline, un saldo local',
    'Unified Offline / Survival OST Coin': 'Moneda OST unificada offline / supervivencia',
    'Mint one offline OST bearer coin for long-term survival, everyday paper or NFC handoff, and provably fair offline games. One vault, one local balance, one redemption path when the network returns.':
      'Minta una moneda OST al portador para supervivencia a largo plazo, traspasos diarios en papel o NFC y juegos justos verificables sin internet. Una bóveda, un saldo local y una ruta de redención cuando vuelva la red.',
    'Catastrophe-Ready': 'Lista para catástrofes',
    'Paper Bearer Notes': 'Notas al portador en papel',
    'Air-Gapped NFC': 'NFC aislado',
    'Satellite Redemption': 'Redención por satélite',
    'LoRa Mesh Relay': 'Relevo de malla LoRa',
    'EMP / Solar Flare': 'EMP / llamarada solar',
    'Global Internet Kill': 'Apagón global de internet',
    'Grid Collapse': 'Colapso eléctrico',
    'Cyber-War': 'Ciberguerra',
    'Mars Colony Init': 'Inicio de colonia en Marte',
    'Air-Gapped NFC Cards': 'Tarjetas NFC aisladas',
    'Mint QR-coded bearer tokens you can print, hand off, and trade like physical cash. No phone, no battery, no internet. Each note carries encrypted proof of value + a one-time redemption hash.':
      'Minta tokens al portador con QR que puedes imprimir, entregar y comerciar como efectivo físico. Sin teléfono, sin batería, sin internet. Cada nota lleva una prueba cifrada de valor y un hash de redención de un solo uso.',
    '✔ Transferable by hand — like real banknotes': '✔ Transferible en mano — como billetes reales',
    '✔ Print on any home printer or copy by hand': '✔ Imprime en cualquier impresora doméstica o copia a mano',
    '✔ Encrypted: useless without the secret key': '✔ Cifrado: inútil sin la clave secreta',
    '✔ Denominated in OST — any amount': '✔ Denominado en OST — cualquier monto',
    'Store OST on passive NFC tags or cold-storage smart cards. Tap-to-verify with any NFC reader. No internet needed — the value lives on the physical card.':
      'Guarda OST en etiquetas NFC pasivas o tarjetas inteligentes de almacenamiento frío. Toca para verificar con cualquier lector NFC. No se necesita internet: el valor vive en la tarjeta física.',
    '✔ Works without power (passive NFC)': '✔ Funciona sin energía (NFC pasivo)',
    '✔ Tamper-evident seal with hash verification': '✔ Sello antimanipulación con verificación de hash',
    '✔ Stackable: load multiple denominations': '✔ Apilable: carga varias denominaciones',
    '✔ Survives water, fire (metal NFC tags)': '✔ Resiste agua y fuego (etiquetas NFC metálicas)',
    'When any node — Solana, OST mesh, or LEO satellite — comes back online, bearer tokens auto-settle on-chain. The 0.1% fee funds satellite rebuilding.':
      'Cuando cualquier nodo — Solana, la malla OST o un satélite LEO — vuelve en línea, los tokens al portador se liquidan on-chain. La comisión de 0.1% financia la reconstrucción satelital.',
    '✔ Redemption queued locally until network returns': '✔ Redención en cola local hasta que vuelva la red',
    '✔ LoRa/Bluetooth mesh bridges gaps': '✔ La malla LoRa/Bluetooth cubre las brechas',
    '✔ Satellite constellation provides last-resort uplink': '✔ La constelación satelital ofrece enlace de último recurso',
    '✔ Zero-knowledge proof preserves privacy on redemption': '✔ La prueba de conocimiento cero preserva la privacidad al redimir',
    'How Survival Tokens Work': 'Cómo funcionan los tokens de supervivencia',
    'Lock OST into bearer vault': 'Bloquea OST en la bóveda al portador',
    'Debit your wallet balance into an on-chain escrow. A unique hash is generated. The tokens are now “burned” from the public ledger.':
      'Debita el saldo de tu billetera hacia un escrow on-chain. Se genera un hash único. Los tokens quedan “quemados” del registro público.',
    'Generate offline bearer token': 'Genera el token offline al portador',
    'A QR code (or NFC payload) containing the encrypted hash + amount + one-time secret is created. This is your physical money.':
      'Se crea un código QR (o carga NFC) con el hash cifrado, el monto y un secreto de un solo uso. Ese es tu dinero físico.',
    'Transfer freely — no network needed': 'Transfiere libremente — sin red',
    'Hand the paper note to someone. Show the QR. Tap the NFC card. The recipient now holds the value. No internet, no phone, no verification step required.':
      'Entrega la nota de papel. Muestra el QR. Toca la tarjeta NFC. La persona receptora ahora tiene el valor. Sin internet, sin teléfono y sin paso de verificación.',
    'Redeem when civilization reboots': 'Redime cuando la red vuelva',
    'Scan the bearer token into any OST client. The vault unlocks, the hash is burned, and the OST credits your wallet. 0.1% fee funds satellite infrastructure rebuilding.':
      'Escanea el token al portador en cualquier cliente OST. La bóveda se desbloquea, el hash se quema y el OST se acredita a tu billetera. La comisión de 0.1% financia infraestructura satelital.',
    'Mint Offline Survival OST': 'Mintar OST offline de supervivencia',
    'Create one OST-BEARER-V1 coin you can print, store on NFC, save as a file, or import into the same local vault used by offline fair games.':
      'Crea una moneda OST-BEARER-V1 que puedes imprimir, guardar en NFC, guardar como archivo o importar en la misma bóveda local usada por juegos justos offline.',
    'Output Format': 'Formato de salida',
    'Paper QR': 'QR en papel',
    'NFC Card': 'Tarjeta NFC',
    'Digital File': 'Archivo digital',
    'Secret Passphrase (optional — adds encryption layer)': 'Frase secreta (opcional — agrega una capa de cifrado)',
    'Leave blank for standard bearer note': 'Déjalo en blanco para una nota al portador estándar',
    'Bearer Amount': 'Monto al portador',
    'Vault Lock Fee': 'Comisión de bloqueo de bóveda',
    'Redemption Fee (later)': 'Comisión de redención (después)',
    'Satellite Fund': 'Fondo satelital',
    'Total Debit': 'Débito total',
    'Mint Offline OST Coin': 'Mintar moneda OST offline',
    'Locking OST in bearer vault…': 'Bloqueando OST en la bóveda al portador…',
    'Generating cryptographic hash…': 'Generando hash criptográfico…',
    'Encrypting bearer payload…': 'Cifrando carga al portador…',
    'Creating offline token…': 'Creando token offline…',
    'Complete ✓': 'Completado ✓',
    'Offline Survival OST Minted': 'OST offline de supervivencia mintado',
    'This bearer coin is now offline-ready. Print it, store it, hand it off, or import it into the local game vault below.':
      'Esta moneda al portador ya está lista sin internet. Imprímela, guárdala, entrégala o impórtala en la bóveda local de juegos abajo.',
    'PAPER BEARER NOTE': 'NOTA AL PORTADOR EN PAPEL',
    'NFC CARD TOKEN': 'TOKEN EN TARJETA NFC',
    'DIGITAL FILE': 'ARCHIVO DIGITAL',
    'Encrypted · One-time redemption': 'Cifrado · redención de un solo uso',
    'Satellite-redeemable': 'Redimible por satélite',
    'Print Bearer Note': 'Imprimir nota al portador',
    'Copy Hash': 'Copiar hash',
    'Download Digital File': 'Descargar archivo digital',
    'Mint Another Offline Coin': 'Mintar otra moneda offline',
    'Unified offline / survival vault': 'Bóveda unificada offline / supervivencia',
    'Your Survival OST Vault': 'Tu Bóveda Survival OST',
    'Mint or import one OST-BEARER-V1 coin from paper QR, NFC card, or digital file. The encrypted local vault below is the same balance used by offline fair games and the same proof queue that syncs when internet returns.':
      'Minta o importa una moneda OST-BEARER-V1 desde QR en papel, tarjeta NFC o archivo digital. La bóveda local cifrada de abajo es el mismo saldo usado por juegos justos offline y la misma cola de pruebas que sincroniza cuando vuelve internet.',
    'OST Offline': 'OST offline',
    'Mint / import bearer coin': 'Mintar / importar moneda al portador',
    'Scan paper QR': 'Escanear QR en papel',
    'Tap NFC card': 'Tocar tarjeta NFC',
    'Import digital file': 'Importar archivo digital',
    'Paste token': 'Pegar token',
    'Import latest minted coin': 'Importar última moneda mintada',
    'Paste OST-BEARER-V1 JSON or the downloaded Survival OST coin text...': 'Pega el JSON OST-BEARER-V1 o el texto descargado de la moneda Survival OST...',
    'Import pasted coin': 'Importar moneda pegada',
    'Offline fair games balance': 'Saldo para juegos justos offline',
    'Coins': 'Monedas',
    'Queued proofs': 'Pruebas en cola',
    'Mode': 'Modo',
    'Manual import': 'Importación manual',
    'Active in games': 'Activo en juegos',
    'Use survival vault in games': 'Usar bóveda survival en juegos',
    'Use faucet balance in games': 'Usar saldo del grifo en juegos',
    'Sync queued proofs': 'Sincronizar pruebas en cola',
    'No survival vault activity yet.': 'Aún no hay actividad en la bóveda survival.',
    'Close scanner': 'Cerrar escáner',
    'Offline Coins Minted': 'Monedas offline mintadas',
    'Offline Value Minted': 'Valor offline mintado',
    'Vault Fee': 'Comisión de bóveda',
    'Offline Lifetime': 'Vida offline',
    'Survival OST vault ready. Mint or import a bearer coin to play fair offline games.':
      'Bóveda Survival OST lista. Minta o importa una moneda al portador para jugar juegos justos offline.',
    'Survival OST vault ready without internet.': 'Bóveda Survival OST lista sin internet.',
    'Survival OST vault already synced.': 'La bóveda Survival OST ya está sincronizada.',
    'Survival OST vault is now the active fair-games balance.': 'La bóveda Survival OST ahora es el saldo activo para juegos justos.',
    'Games are back on faucet play balance.': 'Los juegos vuelven al saldo del grifo.',
    'Offline Survival OST minted. You can import the latest coin into this same vault.':
      'OST offline de supervivencia mintado. Puedes importar la última moneda en esta misma bóveda.',
    'Mint an Offline Survival OST coin first.': 'Primero minta una moneda OST offline de supervivencia.',
    'NFC scan active. Tap an OST bearer card.': 'Escaneo NFC activo. Toca una tarjeta OST al portador.',
    'Camera QR import is not available here. Use file or paste import.': 'La importación QR por cámara no está disponible aquí. Usa archivo o pegado.',
    'Scanner UI missing': 'Falta la interfaz del escáner',
    'Bearer token already imported on this device': 'El token al portador ya fue importado en este dispositivo',
    'No bearer token data found': 'No se encontraron datos de token al portador',
    'Unsupported bearer token format': 'Formato de token al portador no compatible',
    'Web NFC requires Android Chrome 89+. Use file or paste import on this browser.': 'Web NFC requiere Android Chrome 89+. Usa archivo o pegado en este navegador.',
    'LoRa / Bluetooth Mesh Recovery Network': 'Red de recuperación LoRa / Bluetooth Mesh',
    'Even during a total internet blackout, OST bearer tokens can propagate via short-range radio mesh. LoRa gateways relay transactions hop-by-hop until a satellite uplink is found.':
      'Incluso durante un apagón total de internet, los tokens OST al portador pueden propagarse por una malla de radio de corto alcance. Las pasarelas LoRa retransmiten transacciones salto a salto hasta encontrar un enlace satelital.',
    'Phone': 'Teléfono',
    'Gateway': 'Pasarela',
    'Relay': 'Relevo',
    'Satellite': 'Satélite',
    ...
    'Real Physical Cash': 'Efectivo físico real',
    'For the first time, a cryptocurrency can be held as a physical bearer instrument — like a gold coin, but digital, encrypted, and redeemable on any future network.':
      'Por primera vez, una criptomoneda puede sostenerse como instrumento físico al portador: como una moneda de oro, pero digital, cifrada y redimible en cualquier red futura.',
    '2.6B Offline People': '2.6 mil millones de personas sin internet',
    'Right now, 2.6 billion people have zero internet. Survival tokens + LoRa mesh mean they can use OST today — not “someday when Starlink reaches them.”':
      'Ahora mismo, 2.6 mil millones de personas no tienen internet. Los tokens de supervivencia + malla LoRa significan que pueden usar OST hoy, no “algún día cuando Starlink llegue”.',
    'Mars-Ready Currency': 'Moneda lista para Marte',
    'A Mars colony can’t wait 20 minutes for a blockchain confirmation from Earth. Bearer tokens settle locally and sync on the next available uplink.':
      'Una colonia en Marte no puede esperar 20 minutos por una confirmación blockchain desde la Tierra. Los tokens al portador se liquidan localmente y sincronizan con el siguiente enlace disponible.',
    'Survival bearer tokens are experimental. This flow currently generates local bearer previews while on-chain vault locking is under development. Bearer tokens are irreversible once transferred — treat them like cash.':
      'Los tokens de supervivencia al portador son experimentales. Este flujo genera vistas previas locales mientras el bloqueo on-chain de bóveda está en desarrollo. Los tokens al portador son irreversibles una vez transferidos: trátalos como efectivo.',
  ...
  var PATTERNS = [
    [/^Imported\s+(.+?)\s+OST into the Survival OST vault\.$/i,
      function (_m, amount) { return 'Importados ' + amount + ' OST en la Bóveda Survival OST.'; }],
    [/^Survival bearer token minted —\s+(.+?)\s+OST$/i,
      function (_m, amount) { return 'Token de supervivencia mintado — ' + amount + ' OST'; }],
    [/^Bearer hash copied to clipboard$/i, function () { return 'Hash al portador copiado al portapapeles'; }],
    [/^Synced\s+(\d+)\s+proof\(s\)\. On-chain reconciliation queued\.$/i,
      function (_m, n) { return 'Sincronizadas ' + n + ' prueba(s). Reconciliación on-chain en cola.'; }],
    [/^Still offline\.\s+(\d+)\s+proof\(s\) waiting\.$/i,
      function (_m, n) { return 'Todavía sin internet. ' + n + ' prueba(s) esperando.'; }],
    [/^Survival OST vault is now the active fair-games balance\.$/i,
      function () { return 'La Bóveda Survival OST ahora es el saldo activo para juegos justos.'; }],
    [/^⚠\s*Survival bearer tokens are experimental\. This flow currently generates local bearer previews while on-chain vault locking is under development\. Bearer tokens are irreversible once transferred — treat them like cash\.$/i,
      function () { return '⚠ Los tokens de supervivencia al portador son experimentales. Este flujo genera vistas previas locales mientras el bloqueo on-chain de bóveda está en desarrollo. Los tokens al portador son irreversibles una vez transferidos: trátalos como efectivo.'; }],
