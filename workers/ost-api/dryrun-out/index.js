...
async function freeRelayChat(env, { prompt, history = [] } = {}) {
  ...
  if (env && env.GROQ_API_KEY) {
    try {
      ...
      attempts.push({ source: "groq-free", error: publicError(err2) });
    }
  }
  if (env && env.GEMINI_API_KEY) {
    try {
      ...
      attempts.push({ source: "gemini-free", error: publicError(err2) });
    }
  }
  try {
    ...
    attempts.push({ source: "ddg", error: publicError(err2) });
  }
  return {
    text: "I'm here locally, but the model relays did not answer this turn. Groq/Gemini secrets may need rotation, redeploy, or quota recovery.",
    source: "local",
    attempts
  };
}
__name(freeRelayChat, "freeRelayChat");
...
async function callGroq(key, prompt, history) {
  ...
  const data = await r.json().catch(() => ({}));
  if (!r.ok)
    throw new Error("groq " + r.status + " " + String(data.error?.message || "").slice(0, 120));
  return data.choices?.[0]?.message?.content || "";
}
__name(callGroq, "callGroq");
function publicError(err2) {
  return String(err2 && err2.message || err2 || "failed").slice(0, 180);
}
...
async function ddgInstantAnswer(query) {
  ...
  if (data.Heading && data.AbstractURL)
    parts.push(`(${data.Heading} \u2014 ${data.AbstractURL})`);
...
__name(ddgInstantAnswer, "ddgInstantAnswer");

// src/ghost/mesh.js
var PEER_TTL_SEC = 90;
var KEY_PREFIX = "ghost:peer:";
function rid() {
  return "p_" + Math.random().toString(36).slice(2, 10) + Date.now().toString(36);
}
__name(rid, "rid");
async function meshAnnounce(env, body, request) {
  const id = rid();
...
__name(meshAnnounce, "meshAnnounce");
async function meshPeers(env) {
  if (!env || !env.OST_KV)
    return { peers: [] };
  try {
...
__name(meshPeers, "meshPeers");
async function meshBroadcast(_env, body) {
  return { ok: true, fanout: 0, note: "broadcast stub; durable-object fanout lands later", echo: body && body.payload };
}
__name(meshBroadcast, "meshBroadcast");

// src/ghost/memory.js
var KEY = "ghost:journal:v1";
var MAX = 200;
async function memorySave(env, body) {
  if (!env || !env.OST_KV)
...
__name(memorySave, "memorySave");
async function memoryRecent(env) {
  if (!env || !env.OST_KV)
    return { entries: [] };
  try {
    const cur = await env.OST_KV.get(KEY);
...
__name(memoryRecent, "memoryRecent");
...
__name(json, "json");
async function handleGhostV2Request(request, env, { path, method }) {
  if (!path.startsWith("/ghost/v2/"))
    return null;
  if (method === "OPTIONS")
    return new Response(null, { status: 204, headers: CORS_HEADERS });
  try {
    if (path === "/ghost/v2/health") {
      return json({ ok: true, ghost: "v2", ts: (/* @__PURE__ */ new Date()).toISOString() });
    }
    ...
    return json({ error: String(err2 && err2.message || err2) }, { status: 500 });
  }
}
__name(handleGhostV2Request, "handleGhostV2Request");

// src/mesh/identity.js
var KEY_PREFIX2 = "mesh:id:";
var TTL_SECONDS = 60 * 60 * 24 * 7;
function isAddress(addr) {
  return typeof addr === "string" && addr.startsWith("ost-mesh:") && addr.length <= 80;
}
__name(isAddress, "isAddress");
async function identityAnnounce(env, body, ok2, err2) {
  const { address, bundle, fingerprint } = body || {};
  if (!isAddress(address))
    return err2("bad address");
  if (!bundle || bundle.v !== 1)
    return err2("bad bundle");
  if (!bundle.kex || !bundle.sig)
    return err2("missing keys");
  const record = {
    address,
    bundle,
  ...
  await env.OST_KV.put(KEY_PREFIX2 + address, JSON.stringify(record), {
    expirationTtl: TTL_SECONDS
  });
  return ok2({ ok: true, address, ts: record.ts });
}
__name(identityAnnounce, "identityAnnounce");
async function identityLookup(env, address, ok2, err2) {
  if (!isAddress(address))
    return err2("bad address");
  const raw = await env.OST_KV.get(KEY_PREFIX2 + address);
  if (!raw)
  ...
  try {
    ...
    return err2("corrupt record", 500);
  }
}
__name(identityLookup, "identityLookup");

// src/mesh/signal.js
var LEGACY_PREFIX = "mesh:sig:";
var PREFIX = "mesh:sig:v2:";
var TTL_SECONDS2 = 60 * 5;
var MAX_PER_INBOX = 64;
function validAddr(a) {
  return typeof a === "string" && a.startsWith("ost-mesh:") && a.length <= 80;
}
__name(validAddr, "validAddr");
function inboxPrefix(to) {
  return PREFIX + to + ":";
}
__name(inboxPrefix, "inboxPrefix");
function messageId() {
...
__name(legacyKey, "legacyKey");
async function signalSend(env, body, ok2, err2) {
  const { from, to, payload } = body || {};
  if (!validAddr(from) || !validAddr(to))
    return err2("bad addresses");
  if (!payload || typeof payload !== "object")
    return err2("bad payload");
  if (JSON.stringify(payload).length > 32e3)
    return err2("payload too large");
  const ts = Date.now();
  const id = messageId();
  const key = inboxPrefix(to) + ts + ":" + id;
  await env.OST_KV.put(key, JSON.stringify({ id, from, to, ts, payload }), { expirationTtl: TTL_SECONDS2 });
  return ok2({ ok: true, id, ts });
}
__name(signalSend, "signalSend");
async function signalInbox(env, { to, from, since }, ok2, err2) {
  if (!validAddr(to))
    return err2("bad to");
  if (from && !validAddr(from))
    return err2("bad from");
  ...
  if (legacyRaw) {
    try {
      ...
      for (const record of Array.isArray(legacy) ? legacy : []) {
        if (record.ts > minTs && (!from || record.from === from))
  ...
  return ok2({ messages: messages.slice(-MAX_PER_INBOX) });
}
__name(signalInbox, "signalInbox");
...
var ok = /* @__PURE__ */ __name((data, status = 200) => new Response(JSON.stringify(data), {
  status,
  headers: cors({ "Content-Type": "application/json" })
}), "ok");
var err = /* @__PURE__ */ __name((msg, status = 400) => new Response(JSON.stringify({ ok: false, error: msg }), {
  status,
  headers: cors({ "Content-Type": "application/json" })
}), "err");
function cors(extra = {}) {
...
__name(cors, "cors");
async function handleMeshRequest(request, env, { path, method }) {
  if (method === "OPTIONS")
...
var MeshHub = class {
  ...
  signal(body) {
    const { from, to, payload } = body || {};
    if (!validAddr2(from) || !validAddr2(to))
      return fail("bad addresses");
    if (!payload || typeof payload !== "object")
      return fail("bad payload");
    if (JSON.stringify(payload).length > 32e3)
      return fail("payload too large");
    const ts = Date.now();
    const record = { id: messageId2(), from, to, ts, payload };
  ...
  inbox({ to, from, since }) {
    if (!validAddr2(to))
      return fail("bad to");
    if (from && !validAddr2(from))
      return fail("bad from");
    const now = Date.now();
    const minTs = Number(since || 0);
    const inbox = this.inboxes.get(to) || [];
    const messages = [];
    ...
    for (const record of inbox) {
      if (!record || now - Number(record.ts || 0) > SIGNAL_TTL_MS)
        continue;
    ...
    if (keep.length)
      this.inboxes.set(to, keep.slice(-MAX_PER_INBOX2));
    else
...
function normaliseMarket(raw) {
  ...
  return {
    ...
    noPriceNumber: outcomeList.length > 1 && Number.isFinite(outcomeList[1] && outcomeList[1].price) ? outcomeList[1].price : 1 - yesPrice,
    ...
    // ← NEW: full outcomes list for multi-outcome markets
...
__name(cleanProbability, "cleanProbability");
function inferBinaryPrices(side, price, yesPrice, noPrice) {
  ...
  if (yes == null && selected != null)
    yes = sideUp === "NO" ? 1 - selected : selected;
  if (no == null && selected != null)
    no = sideUp === "NO" ? selected : 1 - selected;
  if (selected == null)
    selected = sideUp === "NO" ? no : yes;
  return { price: selected, yesPrice: yes, noPrice: no };
}
__name(inferBinaryPrices, "inferBinaryPrices");
var NATIVE_MARKET_STATE_TTL_S = 60 * 60 * 6;
...
function rememberNativeMarketStateHot(state) {
  ...
  if (NATIVE_MARKET_STATE_MEMORY.size > 250) {
    ...
    if (firstKey)
      NATIVE_MARKET_STATE_MEMORY.delete(firstKey);
  }
...
function readNativeMarketStateHot(marketId) {
  ...
  if (hotAt && Date.now() - hotAt > NATIVE_MARKET_STATE_TTL_S * 1e3) {
...
function rememberRecentPositionHot(record) {
  ...
  for (let i = POSITION_RECENT_MEMORY.length - 1; i >= 0; i--) {
    if (recentPositionKey(POSITION_RECENT_MEMORY[i]) === key)
      POSITION_RECENT_MEMORY.splice(i, 1);
  }
  POSITION_RECENT_MEMORY.unshift(clonePlain(Object.assign({}, record, { hotAt: Date.now() })));
  if (POSITION_RECENT_MEMORY.length > POSITION_RECENT_MEMORY_LIMIT)
...
__name(defaultNativeMarketState, "defaultNativeMarketState");
...
__name(normalizeNativeMarketState, "normalizeNativeMarketState");
...
function positionIsClosed(positionRecord) {
  ...
  if (positionRecord.cashedOut)
    return true;
  if (positionRecord.resolved)
    return true;
  if (Number(positionRecord.cashoutAt ? positionRecord.cashoutAt : 0) > 0)
    return true;
  return ["sold", "cashed-out", "settled", "won", "lost", "closed", "resolved"].indexOf(status) >= 0;
}
__name(positionIsClosed, "positionIsClosed");
...
__name(nativePositionImpact, "nativePositionImpact");
...
__name(applyNativeStateDelta, "applyNativeStateDelta");
...
__name(pruneNativeMarketOrders, "pruneNativeMarketOrders");
...
__name(publicNativeMarketState, "publicNativeMarketState");
...
async function applyNativePositionToMarketState(env, positionRecord, fallbackBaseYes) {
  if (!positionRecord)
    return null;
  if (!isOstNativeMarketId(positionRecord.marketId, positionRecord.source))
    return null;
  ...
  if (previousImpact)
    applyNativeStateDelta(state, previousImpact, -1);
...
function mergeNewest(bucket, record, limit = 100) {
  ...
  const next = key ? current.filter((item) => (item?.signature || item?.sig || item?.id) !== key) : current;
  next.unshift(record);
  return next.slice(0, limit);
}
__name(mergeNewest, "mergeNewest");
function nativeMarketHubStub(env) {
  if (!env || !env.NATIVE_MARKET_HUB)
    return null;
  try {
...
__name(nativeMarketHubStub, "nativeMarketHubStub");
...
async function getNativeMarketStateFromHub(env, marketId, fallbackBaseYes) {
  ...
  if (!cleanMarketId)
    return null;
  const params = new URLSearchParams();
  if (fallbackBaseYes != null)
    params.set("baseYes", String(fallbackBaseYes));
...
__name(getNativeMarketStateFromHub, "getNativeMarketStateFromHub");
async function applyNativePositionToMarketStateHub(env, positionRecord, fallbackBaseYes) {
  if (!positionRecord || !isOstNativeMarketId(positionRecord.marketId, positionRecord.source))
    return null;
  ...
  if (!state)
    return null;
  rememberNativeMarketStateHot(state);
  if (payload.flowRecord)
    rememberRecentPositionHot(payload.flowRecord);
  return state;
}
__name(applyNativePositionToMarketStateHub, "applyNativePositionToMarketStateHub");
...
__name(getNativeRecentPositionsFromHub, "getNativeRecentPositionsFromHub");
var NativeMarketHub = class {
  constructor(state, env) {
    this.state = state;
    this.env = env;
  ...
  async applyPosition(record, fallbackBaseYes) {
    if (!record || !isOstNativeMarketId(record.marketId, record.source))
      return null;
...
function recentFlowRecordForPosition(record) {
  if (!record || !positionIsClosed(record))
    return record;
  const positionKey = nativePositionKey(record);
...
__name(recentFlowRecordForPosition, "recentFlowRecordForPosition");
var FAUCET_WELCOME_AMOUNT = 100;
var FAUCET_DAILY_AMOUNT = 1;
var FAUCET_DAILY_MS = 24 * 60 * 60 * 1e3;
var FAUCET_RESERVATION_MS = 2 * 60 * 1e3;
var STOCK_UNIVERSE = [
...
function parseStockQuoteCsv(text2, symbol) {
  ...
  const row = rows[1];
  const get = /* @__PURE__ */ __name((name) => row[headers.indexOf(name)] || "", "get");
...
__name(parseStockQuoteCsv, "parseStockQuoteCsv");
...
__name(fetchStockQuote, "fetchStockQuote");
async function fetchStockHistory(symbol) {
  ...
  if (chartResponse.ok) {
    try {
      ...
      const history2 = timestamps.map((timestamp, index) => ({
        date: new Date(Number(timestamp) * 1e3).toISOString().slice(0, 10),
      ...
      if (history2.length)
        return { history: history2, source: "yahoo-chart-public" };
    } catch (_) {
    }
  }
  ...
  if (rows.length < 2)
    return { history: [], source: "unavailable" };
  const headers = rows[0].map((h) => h.toLowerCase());
  const idx = /* @__PURE__ */ __name((name) => headers.indexOf(name), "idx");
...
__name(fetchStockHistory, "fetchStockHistory");
async function recordStockWalletEvent(env, order) {
  if (!env.OST_KV || !order || !order.wallet)
    return;
  const record = {
    id: order.id,
    wallet: order.wallet,
...
__name(publicFaucetState, "publicFaucetState");
function faucetWalletKey(wallet) {
  return `faucet:v1:wallet:${wallet}`;
}
__name(faucetWalletKey, "faucetWalletKey");
...
var TOPUP_USD_PER_OST = 0.0118;
var TOPUP_MIN_USD = 1;
var TOPUP_MAX_USD = 5e3;
var TOPUP_TIERS = {
  5: { usd: 5, ostAmount: 1200 },
  10: { usd: 10, ostAmount: 3e3 },
  25: { usd: 25, ostAmount: 9e3 },
  50: { usd: 50, ostAmount: 2e4 }
};
...
async function verifyStripeSignature(rawBody, sigHeader, secret) {
  ...
  if (mac.length !== v1.length)
    return false;
  let diff = 0;
  for (let i = 0; i < mac.length; i++)
    diff |= mac.charCodeAt(i) ^ v1.charCodeAt(i);
  return diff === 0;
}
...
__name(normalizeSolanaCluster, "normalizeSolanaCluster");
function topupCluster(env) {
  return normalizeSolanaCluster(env.TOPUP_SOLANA_CLUSTER || env.SOLANA_CLUSTER || "devnet");
}
__name(topupCluster, "topupCluster");
function topupSolReceiver(env, cluster) {
  const isMainnet = normalizeSolanaCluster(cluster) === "mainnet-beta";
  return isMainnet ? env.TREASURY_SOL_MAINNET || env.TREASURY_SOL_DEVNET || "" : env.TREASURY_SOL_DEVNET || env.TREASURY_SOL_MAINNET || "";
}
__name(topupSolReceiver, "topupSolReceiver");
function topupUsdcReceiver(env, cluster) {
  const isMainnet = normalizeSolanaCluster(cluster) === "mainnet-beta";
  return isMainnet ? env.TREASURY_USDC_MAINNET || env.TREASURY_USDC_DEVNET || topupSolReceiver(env, cluster) : env.TREASURY_USDC_DEVNET || env.TREASURY_USDC_MAINNET || topupSolReceiver(env, cluster);
}
__name(topupUsdcReceiver, "topupUsdcReceiver");
...
__name(topupUsdcMint, "topupUsdcMint");
...
__name(topupRpcUrls, "topupRpcUrls");
...
__name(fetchSolUsd, "fetchSolUsd");
...
__name(solanaRpc, "solanaRpc");
function accountKeyAt(tx, index) {
  const keys = tx?.transaction?.message?.accountKeys || [];
  const key = keys[index];
  return typeof key === "string" ? key : key?.pubkey || "";
}
__name(accountKeyAt, "accountKeyAt");
...
__name(collectParsedInstructions, "collectParsedInstructions");
function transactionHasMemo(tx, memo) {
  ...
  for (const instruction of collectParsedInstructions(tx)) {
    ...
    if (instruction?.program === "spl-memo" || programId.startsWith("Memo")) {
      ...
      if (typeof parsed === "string" && parsed.includes(needle))
        return true;
      if (parsed && typeof parsed === "object" && String(parsed.memo || parsed.text || "").includes(needle))
        return true;
    }
  }
...
__name(transactionHasMemo, "transactionHasMemo");
function sumSolLamportsTo(tx, receiver) {
  let total = 0;
  for (const instruction of collectParsedInstructions(tx)) {
    const parsed = instruction?.parsed;
    if (instruction?.program !== "system" || !parsed || parsed.type !== "transfer")
      continue;
    const info = parsed.info || {};
    if (String(info.destination || "") === receiver)
      total += Number(info.lamports || 0);
  }
  return total;
}
__name(sumSolLamportsTo, "sumSolLamportsTo");
function tokenAmountFromInfo(info) {
  const tokenAmount = info?.tokenAmount || info?.uiTokenAmount;
  if (tokenAmount && Number.isFinite(Number(tokenAmount.uiAmount)))
...
function sumUsdcToTreasury(tx, treasuryOwner, usdcMint) {
  ...
  for (const instruction of collectParsedInstructions(tx)) {
    const parsed = instruction?.parsed;
    const info = parsed?.info || {};
    if (instruction?.program !== "spl-token" || !parsed)
      continue;
    if (parsed.type !== "transfer" && parsed.type !== "transferChecked")
      continue;
    if (info.mint && info.mint !== usdcMint)
      continue;
    if (!treasuryTokenAccounts.has(String(info.destination || "")))
      continue;
...
__name(sumUsdcToTreasury, "sumUsdcToTreasury");
async function verifyCryptoTopupSignature(env, intent, signature) {
  ...
  if (tx?.meta?.err)
    return { ok: false, error: "transaction_failed" };
  if (!transactionHasMemo(tx, intent.memo))
  ...
  const usdcPaid = usdcAmount + 1e-6 >= usdDue;
  let solPaid = false;
  let solUsd = null;
  if (solLamports > 0) {
  ...
  return {
    ...
    rail: usdcPaid ? "usdc-solana-" + (cluster === "mainnet-beta" ? "mainnet" : "devnet") : "sol-solana-" + (cluster === "mainnet-beta" ? "mainnet" : "devnet"),
    solLamports,
    usdcAmount,
    solUsd,
    slot: tx.slot || null
...
__name(findCryptoTopupPayment, "findCryptoTopupPayment");
function adminAuthorized(request, env) {
  if (!env.TOPUP_ADMIN_TOKEN)
    return false;
  ...
  async fetch(request, env) {
    ...
    if (path === "/btc/ticks" && method === "GET") {
      ...
      if (isCurrentRound && ring.length < 2) {
        ...
        const synthetic = [];
        if (Number.isFinite(openPrice) && openPrice > 1e3) {
          synthetic.push({ t: round.openAt, p: openPrice, s: snapshot.openPriceSource || "binance-kline" });
        }
        if (Number.isFinite(livePrice) && livePrice > 1e3) {
          synthetic.push({ t: Math.max(round.openAt + 1, liveTs), p: livePrice, s: snapshot.livePriceSource || snapshot.source || "binance" });
        }
        if (synthetic.length) {
          const seen = /* @__PURE__ */ new Set();
          ring = ring.concat(synthetic).filter((t) => Number.isFinite(Number(t && t.t)) && Number.isFinite(Number(t && t.p)) && Number(t.p) > 1e3).sort((a, b) => Number(a.t) - Number(b.t)).filter((t) => {
    ...
    if (path === "/launchpad/coins" && method === "POST") {
      ...
      const record = {
        ...
        desc: String(desc || "").slice(0, 1e3),
        image: image ? String(image).slice(0, 5e5) : null,
    ...
    if (path === "/topup/intent" && method === "POST") {
      ...
      const intent = {
        ...
        method: method2,
        status: "pending",
        // pending → paid → sent (or cancelled)
        signature: null,
        createdAt: Date.now(),
        updatedAt: Date.now()
      };
