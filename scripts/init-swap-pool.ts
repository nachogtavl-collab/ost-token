...
// =============================================================================
// Creates / refills a "swap pool" devnet keypair that the website can co-sign
// atomic SOL -> OST swaps with. The keypair's secret key is published into the
// site bundle as site/swap-pool.js so the browser can sign on the user's behalf
// to release OST in the same transaction the user pays SOL.
//
// THIS IS A DEVNET-ONLY PATTERN. Anyone with the published secret key could
// drain the swap pool. That's acceptable for devnet (we just refill it) but
// MUST NOT be used on mainnet. For mainnet, replace this with a real on-chain
// swap program or a server-signed flow.
//
// Usage:
...
async function main() {
  ...
  // 4) Write site/swap-pool.js (publishes the secret key — DEVNET ONLY)
  const sitePath = path.join(__dirname, '..', 'site', 'swap-pool.js');
  const secretArray = Array.from(pool.secretKey);
  const banner = '/* AUTO-GENERATED by scripts/init-swap-pool.ts — DEVNET ONLY. Do NOT use this pattern on mainnet. */\n';
  const body =
  ...
  fs.writeFileSync(sitePath, banner + body);
  console.log(`Wrote ${sitePath}`);

  console.log('\nDone. Refresh the site to pick up the new swap pool.');
}
