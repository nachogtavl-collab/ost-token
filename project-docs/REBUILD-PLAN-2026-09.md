...
- **Subscriber-less poll removed:** `prediction-pro.js broadcast()` fetched `/markets` + Polymarket every 30s in every tab with zero subscribers (nothing calls `subscribe`). Now runs only with a subscriber and a visible tab. Steady state measured 11.5 -> **6.0 req/min** (boot 35). Remaining steady pollers, all ~60s: `/ost/price`, `/launchpad/coins`, `/positions/recent` (x2 modules), `/play/reconcile`, `/markets/state/*`, `/stocks/quotes`, `/ost/stats`, mesh announce. Next: fold price + market state + recent positions into the existing socket push, dedupe the two `/positions/recent` callers.

### 2026-09-18 — Phase 2 polling: steady state 6.0 -> 0.5 req/min

- **Market state rides the push:** NativeMarketHub's 5s round push now carries `marketState` (one DO storage read) and only the last 12 ticks (was 120 per push); `prediction-pro.js` remembers it and marks it fresh, so the 60s `/markets/state/*` poll is silent while the socket delivers.
- **On-screen-only pollers:** `/launchpad/coins` (was every minute all session), `/play/reconcile`, `/positions/recent` catch-up (now 3 min), `/ost/stats`; `/ost/price` 60s -> 180s and never while hidden.
...
### 2026-09-25 — OUTAGE: every buy/sell/faucet/bridge/deposit crashed for a week (my regression) + trade pricing

- **Root cause (regression from e32dd41, 2026-09-18):** the mesh CORS patch regex appended `, x-mesh-*` after every `x-ost-internal'` in index.js - including 3 places where it was a HEADER NAME in code (`request.headers.get('x-ost-internal, x-mesh-addr, ...')` in walletAuthGuard, and two internal fetch header keys). An invalid header name throws, so every wallet-signed POST returned Cloudflare 1101 (uncaught exception): faucet reserve/commit, ATA rent, cosign (bridge), /play deposit, predict open/cashout, payouts. Fixed; unsigned now correctly 401s. **Lesson:** never regex-patch a token that appears both in CORS strings and in code; after ANY worker deploy run the real-money funnel (claim -> bridge -> deposit -> buy -> sell), not just the feature's own test.
- **Trades priced on a stale BTC price:** open/cashout read `btc:latest`, which is written in the NativeMarketHub isolate's memory + a throttled KV copy (TTL 30 min). They now use the canonical round's live price (forced fresh per trade), fall back to a KV tick only if <20s old, else refuse `price_unavailable`. Verified mid-round: odds 83%, desk offered 4.98, server paid 4.997.
- **Sold position reappeared:** `patchPredictionOrderByRef` matched only `reference`; native orders are keyed by signature/id, so the patch silently returned false and the desk kept showing the sold position. Now matches any identity; the desk records the order if the patch misses, and clears the card on a confirmed sale.
- **sw.js precache drift:** 7 entries behind index.html (incl. bridge + session-key); synced.
- **Wallet tab on phones** now opens on tabs + address/balances (marketing hero hidden), profile card no longer one-char-per-line.
- **Verified e2e on the live site (headless iPhone, real devnet funds):** wallet ~1s, claim 100 OSTC ~9s, bridge 20, deposit 10, buy 3 (UI 7 = server 7), sell (UI = server), card clears, 0 page errors.

### 2026-09-25 (b) — External audit, critical + high items

- **CRITICAL - no live price (root cause):** Coinbase's `api.coinbase.com/v2/prices/BTC-USD/spot` returned ONE frozen price for minutes while the exchange traded ~$30 away, and it was the worker's de-facto settlement source (Binance 451s Cloudflare). Every round showed price == beat, a flat chart and 50/50. Worker feed order is now Coinbase Exchange ticker -> Kraken -> Binance Vision -> Binance -> Coinbase v2 -> CoinGecko, plus a **freeze detector** (identical print for 45s+ = source skipped). Verified: live price moves every tick, odds track it.
- **Browser feeds:** removed browser->exchange fetches, the Binance WebSocket and the public CORS proxies (corsproxy.io / allorigins: 401s + third-party exposure); the direct fallback now asks the OST worker. Pyth Hermes now requires an API key (401 without CORS headers = opaque failure) -> browser Pyth client is off unless `window.OST_PYTH_KEY` is set. Console: 0 price-feed failures (was ~20/page).
- **Trade gate:** on a live BTC round with no settlement tick for 20s, Buy buttons disable, read "Price unavailable / Trading paused", the ticket confirm is disabled, and confirmBuy refuses (server also refuses `price_unavailable`).
- **Numbers you can't trust:** wallet tiles had two writers each (app.js legacy treasury 28.12M + legacy claim count 3 vs live-stats pool 9.52B + active wallets). One owner (live-stats), honest labels "Payout pool (OSTC)" / "Active wallets (24h)", "…" until loaded. Desktop header badge showed OSTC + retired credits ("25.00 OST") -> now on-chain OSTC only, labelled OSTC. Predictions fiat hint repaints on currency change (was stuck on INR from onboarding). Home ticker relabelled "Devnet test index" (no cash value); Predictions subtitle "Real-world markets · devnet OSTG (no cash value)".
- **Known:** sell pays the server's price at fill; the desk's displayed quote can lag it by ~1-2 odds points (seen: shown 2.93, paid 3.05). Next: show the server's quote in the sell ticket.
- Medium/low audit items (hash navigation, desktop floating panels, top-menu overflow, phone pop-ups, predictions inset, "Руский", transmit placeholders, headings/alt, page weight) are next.
- **Audit medium/low (done):** `#markets` deep link (aliases `#live-bet`, `#predictions`, `#wallet-panel-predict`) opens Predictions on phone + desktop, Markets tab leaves `#markets` in the URL, desktop `#wallet` not blank (verified 5 cases). Predictions flush on phones (grid 277 -> 362px). Unsolicited ghost bubble + achievement toasts hidden on phones. Desktop: new `docs/ost-desktop-nav.js` adds Markets + Games to the top bar, fits links + "More ▾" (was 18 links in 755px behind a scrollbar), hides decorative/unsolicited widgets (Veil pill, ghost greeting, section pill, streak badge), lays the 5 launchers out in one non-overlapping row, dock ends above it. "Русский" fixed, one h1, labelled Transmit inputs.
- **Still open from the audit:** page weight (129 scripts / ~10k nodes / 250 requests - lazy-load closed sections), 3 broken demo-shop images, 2 JS-injected images without alt, sell ticket should show the server's quote.
